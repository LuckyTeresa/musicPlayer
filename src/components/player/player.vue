 ...
 >
      <div class="normal-player" v-show="fullScreen">
        ...
        <div class="progress-wrapper">
            ...
            <div class="progress-bar-wrapper">
                <progressbar :percent='percent' @percentChange='onPregressBarChange'></progressbar>
            </div>
    ...
    <transition name='mini'>
      <div class="mini-player" v-show="!fullScreen" @click='open'>
        ...
        <!-- 阻止miniIcon的点击事件冒泡到mini-player 而使播放页面弹出 -->
  ...
  <!-- audio标签从加载到可以播放时会触发canplay事件 -->
  <audio :src='currentSong.url' ref='audio' @play='ready' @ended='end' @error='error' @timeupdate='updateTime'></audio>
</div>
</template>
...
export default {
  ...
  computed:{
    ...mapGetters(['fullScreen','playing','currentIndex','playList','currentSong','sequenceList'
        ]),
    playIcon(){
  ...
  methods:{
    ...
    enter(el,done){
        ...
        animations.registerAnimation({//注册动画
            name:'move',animation,presets:{duration:400,easing:'linear'}
        })
    ...
    loop(){
        ...
        // 如果处于循环模式,那么在歌曲放完之后将歌词也重置到最开始的位置
        if(this.currentLyric){
    ...
    middleTouchMove(e){
        ...
        middleTouchEnd(e){
            ...
            if(this.currentShow=='cd'){//在cd页面向左划
  ...
  watch:{
        currentSong(newSong,currentSong){
          if(!newSong.id){
            ...
            if(newSong.id==currentSong.id){//当前歌曲没有变化,下面的操作不执行
                return
            }
